...
#import "WPCaptureSessionCoordinator.h"

@interface WPCaptureSessionCoordinator ()
...
@implementation WPCaptureSessionCoordinator
- (instancetype)init
{
    self = [super init];
    if(self){
        _sessionQueue = dispatch_queue_create("cwp.capturepipeline.session", DISPATCH_QUEUE_SERIAL);
...
- (AVCaptureSession *)setupCaptureSession
{
    ...
    captureSession.sessionPreset = AVCaptureSessionPreset640x480; //设置录制视频分辨率
...
- (BOOL)cutCameraDeviceFromeCaptureSession:(AVCaptureSession *)captureSession
{
    ...
    for (input in inputs) {
        if ([input.device isEqual:self.cameraDevice]) {
    ...
    return [self addCameraAtPosition:position toCaptureSession:captureSession];
}
